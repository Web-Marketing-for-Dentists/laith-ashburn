...
const POST = async ({ request, locals }) => {
  ...
  const mailgun = async (to, subject, html) => fetch(`https://api.mailgun.net/v3/${domain}/messages`, { method: "POST", headers: { Authorization: `Basic ${btoa(`api:${apiKey}`)}`, "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ from, to, subject, html }).toString() });
