...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const mailgun = async (to: string, subject: string, html: string) => fetch(`https://api.mailgun.net/v3/${domain}/messages`, { method: "POST", headers: { Authorization: `Basic ${btoa(`api:${apiKey}`)}`, "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ from, to, subject, html }).toString() });
  ...
  if (!patient.ok || !admin.ok) return new Response(JSON.stringify({ error: "Email delivery failed." }), { status: 502, headers: { "Content-Type": "application/json" } });
